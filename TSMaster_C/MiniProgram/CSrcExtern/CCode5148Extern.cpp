...
DLLEXPORT s32 __stdcall retrieve_mp_abilities(const void* AObj, const TRegTSMasterFunction AReg) {
  ...
  if (!AReg(AObj, "step_function", "step", "10", reinterpret_cast<const void*>(&step), "")) return -1;
  if (!AReg(AObj, "var", "TxMsg", "3,ezAsMHgxLDgsMCwweDAsMCx7MHgwLCAweDAsIDB4MCwgMHgwLCAweDAsIDB4MCwgMHgwLCAweDB9fQ__,", reinterpret_cast<const void*>(&TxMsg), "")) return -1;
  if (!AReg(AObj, "timer", "SendTimer1", "1000", reinterpret_cast<const void*>(&SendTimer1), "")) return -1;
  if (!AReg(AObj, "on_can_rx_callback", "on_can_rx_CAN_Rx1", "291,-1,0", reinterpret_cast<const void*>(&on_can_rx_CAN_Rx1), "")) return -1;
  if (!AReg(AObj, "on_timer_callback", "on_timer_Send_Timer1", "SendTimer1", reinterpret_cast<const void*>(&on_timer_Send_Timer1), "")) return -1;
  if (!AReg(AObj, "on_start_callback", "on_start_Send_Init", "", reinterpret_cast<const void*>(&on_start_Send_Init), "")) return -1;
  if (!AReg(AObj, "on_stop_callback", "on_stop_Send_Stop1", "", reinterpret_cast<const void*>(&on_stop_Send_Stop1), "")) return -1;
